# -*- coding: utf-8 -*-
"""
Created on Mon Feb 17 20:33:08 2017
...
def undistort(image, show=True, read = True):
    '''
    distortion correction: calculate camera calibration matrix and distortion coefficients to remove distortion
    ''' 
    if read:
...
def birds_eye(img, display=True, read = True):
    '''
    Persepctive transform: make lane lines to display parallelly to fit polynomials to lane lines and measure the curvation 
    '''
    if read:
...
def apply_thresholds(image, show=True):
    '''
    binary thresholds to identify the lane lines
    '''
    img, M = birds_eye(image, display = False)

    s_channel = cv2.cvtColor(img, cv2.COLOR_BGR2HLS)[:,:,2] #S channel from HLS (180, 255)
    
    l_channel = cv2.cvtColor(img, cv2.COLOR_BGR2LUV)[:,:,0] #L channel from LUV (225, 255)

    b_channel = cv2.cvtColor(img, cv2.COLOR_BGR2Lab)[:,:,2] #B channel from Lab (155, 200)
...
def fill_lane(image):
    '''
    isolate lane line poxels and fit a polynomial to the lane lines
    ...
    center = abs(640 - ((rightx_int+leftx_int)/2)) #average of the x intercepts of each line
    ...
    if center < 640:
        ax2.text(200, 100, 'Vehicle is {:.2f}m left of center'.format(center*3.7/700),
                 style='italic', color='blue', fontsize=12)
    else:
...
class Line:
    '''
    This function is used to store attributes about the lane lines from one frame to the next
    ...
    def found_search(self, x, y):
        '''
        A sliding window to search for lane pixels in close proximity (+/- 25 pixels in the x direction)
    ...
    def blind_search(self, x, y, image):
        '''
        A slinding window approach to detect peaks in a histogram of the
...
def process_vid(image):
    '''
    This function is to process a video frame by frame and outputs an annotated video with lane lines, radius of curvature and distance from center
    '''
...
if __name__ == '__main__':
    ...
    images = glob.glob('D:/git/github/Autonomous_vehicle/4_Selfdriving_advancedlane/Advanced-Lane-Finding/camera_cal/calibration*.jpg')
    ...
    for idx, fname in enumerate(images):
        ...
        if ret == True:
            ...
            ax2.set_title('Chessboard Corners', fontsize=12)
    ...
    Left = Line()
    Right = Line()
    video_output = 'D:/git/github/Autonomous_vehicle/4_Selfdriving_advancedlane/Advanced-Lane-Finding/output_video.mp4'
    clip1 = VideoFileClip("D:/git/github/Autonomous_vehicle/4_Selfdriving_advancedlane/Advanced-Lane-Finding/project_video.mp4").subclip(0,2)
